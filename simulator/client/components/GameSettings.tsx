...
export function ChooseGameSettings() {
    const useGameTarget = (gameSettings: any, id: string, value: any) => {
        gameSettings[id] = value;
        return true;
    };
    const useGameValue = (gameSettings: any, id: string) => gameSettings[id];
    return (
        <Card>
            <CardHeader>
...
export function ChooseTeamSettings() {
    const teamSettingsRef = useRef<any>();
    const gameSettings = useBucket(btGameSettings);

    const renderTeams = () => {
        if(!gameSettings || !gameSettings.teams) return <></>;
        const teams: any[] = gameSettings.teams;
...
function TeamSettings(props: { team_order: number }) {
    const [active, setActive] = useState(false);

    const onChangeOrder = (dir: number) => {
        const gameSettings = btGameSettings.get();
        const teams: any[] = gameSettings.teams;

        teams.sort((a: any, b: any) => a.team_order - b.team_order);

        const activeTeam = teams[props.team_order];
        const otherTeam = teams[props.team_order + dir];
        const otherTeamOrder = otherTeam.team_order;
        const activeTeamOrder = activeTeam.team_order;
    ...
    const isUpActive = teams && props.team_order > 0;
    const isDownActive = teams && props.team_order < teams.length - 1;

    const useTeamTarget = (gameSettings: any, id: string, value: any) => {
        const ts: any[] = gameSettings.teams;
        if (!ts) return false;
        const team = ts[props.team_order];
        if (!team) return false;
        team[id] = value;
        return true;
    };

    const useTeamValue = (gameSettings: any, id: string) => {
        const ts: any[] = gameSettings.teams;
        if (!ts) return;
        const team = ts[props.team_order];
        if (!team) return;
            ...
            <HStack spacing="2rem">
                ...
                >
                    <IconButton
                        ...
                        _active={{ color: isUpActive ? "gray.50" : "gray.850" }}
                        onClick={() => {
                    ...
                    <IconButton
                        ...
                        _active={{ color: isDownActive ? "gray.50" : "gray.850" }}
...
function SettingColorInput(props: {
    ...
    title?: string;
    team_order?: number;
    useTarget?: (gs: any, id: string, value: any) => boolean;
    useValue?: (gs: any, id: string) => any;
    ...
    const isTeamId = "team_order" in props;
    let team_order = -1;
    if (isTeamId) {
        team_order = Number.parseInt(String(props.team_order || 0));
    ...
    useEffect(() => {
        if (!regexColorHex.test(currentValue)) {
            ...
            if (!isTeamId && id in gs) {
                gs[id] = value;
            } else if (isTeamId && gs.teams[team_order]) {
                gs.teams[team_order][id] = value;
            }
...
export function ChooseScreenSettings() {
    try {
        const gameSettings = useBucket(btGameSettings);
                    ...
                    <VStack w="100%">
                        ...
                        >
                            ...
                            <Box
                                ...
                                display={gameSettings?.screentype == 1 ? "none" : "block"}
                                pt="1rem"
                            >
                                <Text
                                    as="label"
                                ...
                                <HStack justifyContent={"center"} alignItems={"center"}>
                                    <SettingNumberInput
                                        ...
                                        useValue={useGameValue}
                                    />
                                    <Text as="span" fontSize="2rem" alignSelf={"flex-end"}>
                                        :
                                    </Text>
                                    <SettingNumberInput
                                        id="resoh"
                            ...
                            <Box
                                pt="1rem"
                                id="viewportSize"
                                display={gameSettings?.screentype != 3 ? "none" : "block"}
                            >
                                <Text
                                    as="label"
                                    display={"inline-block"}
                                ...
                                <HStack pt="0.5rem" justifyContent={"center"} alignItems={"center"}>
                                    <SettingNumberInput
                                        id="screenwidth"
                                        title="Width (px)"
                                        placeholder="800"
