...
function Timeleft() {
    const timeleftUpdated = useBucket(btTimeleftUpdated);
    let timeleft: number = btTimeleft.get() || 0;

    try {
        timeleft = Number.parseInt(String(timeleft)) / 1000;
    } catch (e) {
        timeleft = 0;
    }
    ...
    let ms: string | number = 100 * (timeleft - Math.floor(timeleft));
    if (ms < 10) {
        ms = "0" + ms;
    } else {
        ms = "" + ms;
