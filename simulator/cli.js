...
console.log("[ACOS] Starting ACOS Simulator CLI...");
...
function runServer(isDev) {
    return new Promise(async (rs, rj) => {
        ...
        const cmd = `"${nodemonPath}" --inspect --enable-source-maps --watch ./dist/server --watch ./dist/shared --ignore ./public ./dist/server/server.js "${cwd}" ${env} "${acosClientType}"`;
...
function runTSWatch() {
    return new Promise((rs, rj) => {
        let tscPath = path.join(cd, "node_modules/.bin/tsc");
        if (!fs.existsSync(tscPath)) {
            tscPath = path.join(cd, "../../node_modules/.bin/tsc");
        }
        if (!fs.existsSync(tscPath)) {
            tscPath = "tsc";
        }
        const cmd = `"${tscPath}" -p tsconfig.json --watch --preserveWatchOutput`;

        runScript("tsc-watch", cd, cmd, (err) => {
            if (err) {
                console.error(err);
                rj(err);
                return;
...
const { openBrowser } = require("./dist/server/openBrowser");
